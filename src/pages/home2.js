...
const OpenChargerAPIKey = '4ebfd445-0045-418f-85df-85d035140274';
...
const ChargerMap = () => {
    ...
    L.Control.geocoder({
      ...
      .on('markgeocode', async function (e) {
        ...
        if (routingControlRef.current) {
          routingControlRef.current.spliceWaypoints(0, 1, center);
        } else {
          const startPoint = center;
          const endPoint = [40.748817, -73.985428]; // Example: End point
          const waypoints = [startPoint];

          // Fetch chargers along the route
          const routeChargers = await fetchChargersAlongRoute(startPoint, endPoint);

          waypoints.push(...routeChargers);
    ...
    map.on('click', function (e) {
      if (routingControlRef.current) {
        routingControlRef.current.spliceWaypoints(
          routingControlRef.current.getWaypoints().length - 1,
          1,
          e.latlng
        );
      }
  ...
  const fetchChargersAlongRoute = async (start, end) => {
    try {
      const response = await axios.get(
        `https://api.openchargemap.io/v3/poi?output=json&countrycode=US&latitude=${start.lat}&longitude=${start.lng}&distance=50&maxresults=10&key=${OpenChargerAPIKey}`
      );
      return response.data.map((charger) => ({
        latLng: L.latLng(charger.AddressInfo.Latitude, charger.AddressInfo.Longitude),
      }));
    } catch (error) {
